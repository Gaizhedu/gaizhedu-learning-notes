...
但是由于两个线程想要拿到的锁都被互相占用，就导致了永远也拿不到锁的情况

这种现象就被称为**死锁**

解决的方法也很简单，可以选择按顺序加锁，也就是先所有线程先拿锁A，再拿锁B

这样就不会导致出现死锁的情况

或者可以选择定时放弃，也就是在一定时间后如果没办法拿到全部的锁就自动放弃
...
主要使用到这些方法：`wait()`、`notify()`、`notifyAll()`

这里与上文介绍锁时候提及到的`ReentrantLock`有一些关联

首先先介绍什么是线程间通信，简单来讲就是各个线程之间可以通过一些手段来联系，进而实现更好的调动

### wait()
这个方法的作用是让当前的线程进入**等待**状态，在该状态下，线程会暂时停止，直到有人唤醒

举一个简单的例子，假设有两个线程，一个线程用于生产，一个用于消耗

每次生产（消耗）的时间都是随机的，那么便会出现说生产的多或者消耗的多

那么此时便可以在生产满的时候让该线程进入等待状态，而消耗线程运行后才会唤醒该生产线程

这样做的好处是避免出现一些越界的问题

### notify() & notifyAll()
那么我们要怎么唤醒呢？需要使用到这个方法
